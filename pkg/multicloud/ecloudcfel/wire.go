...
func (w *SWire) CreateINetwork(opts *cloudprovider.SNetworkCreateOptions) (cloudprovider.ICloudNetwork, error) {
	...
	req := NewConsoleRequest(w.vpc.region.ID, "/api/openapi-vpc/customer/v3/network", nil, jsonutils.Marshal(params))
	res, err := w.vpc.region.client.doPost(req)
	if err != nil {
		return nil, err
	}
	return &SNetwork{
