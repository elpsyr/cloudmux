...
const (
	CLOUD_PROVIDER_BAIDU    = "BaiduCfel"
...
func (self *SBaiduClient) getUrl(service, regionId, resource string) (string, error) {
	...
	case "bcm":
		return fmt.Sprintf("https://bcm.%s.baidubce.com/%s", regionId, strings.TrimPrefix(resource, "/")), nil
	case "bos":
...
func (self *SBaiduClient) list(service, regionId, resource string, query map[string]string, params map[string]interface{}) (jsonutils.JSONObject, error) {
	return self.request(httputils.GET, service, regionId, resource, query, params)
}

func (self *SBaiduClient) post(service, regionId, resource string, params map[string]interface{}) (jsonutils.JSONObject, error) {
	return self.request(httputils.POST, service, regionId, resource, nil, params)
}

func (self *SBaiduClient) delete(service, regionId, resource string, params map[string]interface{}) error {
	_, err := self.request(httputils.DELETE, service, regionId, resource, nil, params)
	return err
}

func (self *SBaiduClient) request(method httputils.THttpMethod, service, regionId, resource string, query map[string]string, params map[string]interface{}) (jsonutils.JSONObject, error) {
	...
	// }
	values := url.Values{}
	if method == httputils.GET && len(query) > 0 {
		for k, v := range query {
			values.Set(k, v)
		}
...
func (self *SBaiduClient) getOwnerId() (string, error) {
	...
	resp, err := self.list("bos", "bj", "/", nil, nil)
	if err != nil {
		return "", err
	}
